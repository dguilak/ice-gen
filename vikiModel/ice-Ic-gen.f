c version of mspc for zero dipole
...
c building models for ice Ic bounded by hexagonal surfaces
c CUBIC ice version
...
c fort.3 long log, fort.33 short log
	...
	write(3,*)'possible for ice Ic, given unit cell'
...
c	parameter (nmoll=1000,natmm=3000,ndimm=9000)
c spn
	parameter (nmoll=2500,natmm=4000,ndimm=9000)
...
c unit cell of 2-nd layer
	unc(2,1,2)=a*0.5d0
	unc(2,2,1)=a*sqrt(3.0d0)/2.0d0
	unc(2,2,2)=a
	unc(2,2,3)=roo/3.0d0
	unc(2,3,1)=a*sqrt(3.0d0)/2.0d0
	unc(2,3,2)=a*2.0d0
	unc(2,4,2)=a*2.5d0
	unc(2,4,3)=roo/3.0d0
c unit cell of 3-rd layer
	unc(3,1,1)=a*sqrt(3.0d0)/2.0d0
	unc(3,1,3)=roo/3.0d0
	unc(3,2,1)=a*sqrt(3.0d0)/2.0d0
	unc(3,2,2)=a
	unc(3,3,2)=a*1.5d0
	unc(3,3,3)=roo/3.0d0
	unc(3,4,2)=a*2.5d0
	...
	do 1 iz=0,nz-1
	   ...
	   do 2 ix=0,nx-1
		...
		do 3 iy=0,ny-1
			...
			do 5 k=1,3
5		           xlat(nmol,k)=unc(ila,iat,k)+vv(k)*ixyz(k)
c		   	write(97,*)'iz,ix,iy,iat',iz,ix,iy,iat
c		   	write(97,*)'nmol',nmol
c		   	write(97,*)(xlat(nmol,k)*ams,k=1,3)
4		    continue
3	        continue
2	    continue
1	continue
	print*,'nmol=',nmol
	write(3,*)'nmol=',nmol
	cod=dipli*4.*pi/3.0*nmol/(side(1)*side(2)*side(3))
	write(3,*)'cod=',cod/akm
	print*,'cod kcal/mol=',cod/akm
	natm=nmol*3
	ndim=natm*3
...
c shift by vector dell
	do 6 nm=1,nmol
	   do 7 k=1,3
	   	xlat(nm,k)=xlat(nm,k)+dell(k)*roo
		if(xlat(nm,k).gt.side(k))xlat(nm,k)=xlat(nm,k)-side(k)
		if(xlat(nm,k).lt.0.0d0)xlat(nm,k)=xlat(nm,k)+side(k)
7	   continue
...
305	format(2i5,2a5,3f10.5,2a5,f10.5)
	...
	do 303 nm=1,nmol
	   ...
	   if(iho.eq.1)then
	      do 3306 j=1,3
3306	         ve(j)=xlat(nm,io+j)*ams*0.5
	      write(99,305)kaa,nm,wat,car,ve,wat,car,dble(ia)
	   else
	      do 306 j=1,3
306	         ve(j)=xlat(nm,io+j)*ams
...
c			write(98,*)(xlat(nm,k)*ams,k=1,3)
c			write(98,*)(xlat(nmm,k)*ams,k=1,3)
c		   end if
		   nne(nm)=nne(nm)+1
...
c nsmax is max num of distinct structures expected to be found
	...
	common/param5/roo,rfcc,cutne,ctn2
	common/param6/ksha(3,2)
	common/pottts/eppo(nmoll),eppp,ehh
...
18	   continue
	   sb1=1.0/sqrt(sb)
	   sb=0.5d0*rsu*sb1
	   sh=0.5d0*rhh/sqrt(sh)
	   do 19 j=1,3
		xlat(nm,3+j)=xlat(nm,j)+bis(j)*sb+vhh(j)*sh
		xlat(nm,6+j)=xlat(nm,j)+bis(j)*sb-vhh(j)*sh
...
c translate to system of dipoles
	...
	do nm=1,nmol
	   ...
	   do 155 k=1,4
		...
		ndip=ndip+1
		do j=1,3
		   xld(ndip,j)=xlat(nm,j)+0.5*xnei(nm,k,j)
...
155	   continue
	...
	epo(nst,3)=ed1
	epo(nst,4)=dip(4)
	write(33,3334)ep1,eh1,ed1,dip(4),icat,nst
	close (33)
...
3334	   format(f8.3,2e12.4,e12.5,4i4,i6)
	end do
...
1003	continue
	...
	write(3,*)'lowest energy tot/ehh/edip/dipole'
	...
	do i=1,nst
	   ...
	   write(35,666)epo(i,3)-epo(1,3),epo(i,4),ra
...
c spn
	...
	i2=(k2-1)*3
	do 10 k=1,3
	   ve(k)=xlat(nm,i1+k)-xlat(nmm,i2+k)
	   if(ve(k).lt.-side(k)*0.5d0)ve(k)=ve(k)+side(k)
	   if(ve(k).gt.side(k)*0.5d0)ve(k)=ve(k)-side(k)
10	continue
...
c spn
	...
	common/parambo/side(3)
	dimension dip(n4)
	if(dip(4).lt.1.d-4)then
	write(9,*)nmol,ep1,(dip(k),k=1,4),(side(k),k=1,3)
	do 1 nm=1,nmol
	   write(9,*)(xlat(nm,k),k=1,9)
1	continue
	stop
	end if
c	close (9)
c	open(file='fort.9',form='formatted',status='old',
c     x	unit=9,access='append')
	return
	end
	subroutine epot
...
c nearest neighbors
		   ...
		   if(ae.lt.-5.5.and.ae.gt.-5.8)icat(2)=icat(2)+1
		   if(ae.lt.-4.6.and.ae.gt.-4.9)icat(3)=icat(3)+1
		   if(ae.lt.-4.3.and.ae.gt.-4.6)icat(4)=icat(4)+1
		end if
1202	        continue
	      end if
	    end do

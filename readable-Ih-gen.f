...
c nmoll max num of water molecules
	...
	common /unitc/ erg_au,ev_au,cm1_au,kelvin_au,kcalmol_au,milliev_au,amu_au,angstr_au,au_sec,au_picosec
	common/xmmmmm/ atom_mass(3),xmass(9),xiner_inertia(3),atom_mass_sum
	common/param1/oh_dist,hoh_angle,tips2_hchg,ao2,co2,rom,qmm,qmh,qhh,ff,ff12,ddpl
	common/param2/chap(3,3),a12,c6,fam(9),rhh,rsu,pi,cut2,potent_cutoff,dipli
	common/param6/ksha(3,2)
	common/param3/xredi(3),xdis2(3),xdee(3)
	common/param5/roo,rfcc,nn_cutoff,nn_cutoff_sq
	common/parambo/side(3)
	common/numbs/nmol,natm,ndim
	dimension dell(3),lattice_vec(3),unit_cell(3,4,3)
	dimension ixyz(3)
...
c lattice vectors
	lattice_vec(1)=roo*4.0d0/sqrt(6.0d0)
	lattice_vec(2)=a*3.0d0
	lattice_vec(3)=roo*(1.0d0+1.0d0/3.0d0)
	print*,'8 atoms in a cell'
	print*,'unit vectors in angstr_au:'
	print*,(lattice_vec(k)*angstr_au,k=1,3)
	write(3,*)'a=',a*angstr_au,' lattice_vec=',(lattice_vec(k)*angstr_au,k=1,3)
c700	print*,'layersx,layersy,layersz?'
c	print*,'layersz=#hex layers must be multiple of 2'
c	print*,'layersx probably should be > 2'
c number of repeating units in each direction
        layersx=anx
        layersy=any
        layersz=anz
c	read*,layersx,layersy,layersz
c	if(amin0(layersx,layersy,layersz).lt.2)then
c	   print*,'too small layersx or layersy or layersz'
c	   stop
c	end if
	side(1)=layersx*lattice_vec(1)
	side(2)=layersy*lattice_vec(2)
	side(3)=layersz*lattice_vec(3)
	print*,'sides of box A',side(1)*angstr_au,side(2)*angstr_au,side(3)*angstr_au
c	print*,'satisfied? enter 0'
c	read*,isat
c	if(isat.ne.0)go to 700
	write(3,*)'layersx,layersy,layersz',layersx,layersy,layersz
	write(3,*)'sides of box A',side(1)*angstr_au,side(2)*angstr_au,side(3)*angstr_au
c location of atoms in surface unit cell
	do 100 i=1,3
	  do 100 j=1,4
	   do 100 k=1,3
100		unit_cell(i,j,k)=0.0d0
c unit cell of bottom layer
	unit_cell(1,1,1)=a*sqrt(3.0d0)/2.0d0
	unit_cell(1,2,2)=a*0.5d0
	unit_cell(1,2,3)=roo/3.0d0
	unit_cell(1,3,2)=a*1.5d0
	unit_cell(1,4,1)=a*sqrt(3.0d0)/2.0d0
	unit_cell(1,4,2)=a*2.0d0
	unit_cell(1,4,3)=roo/3.0d0
c unit cell of 2-nd layer
	unit_cell(2,1,1)=a*sqrt(3.0d0)/2.0d0
	unit_cell(2,1,3)=roo/3.0d0
	unit_cell(2,2,2)=a*0.5d0
	unit_cell(2,3,2)=a*1.5d0
	unit_cell(2,3,3)=roo/3.0d0
	unit_cell(2,4,1)=a*sqrt(3.0d0)/2.0d0
	unit_cell(2,4,2)=a*2.0d0
	...
	do 1 iz=0,layersz-1
	   ...
	   do 2 ix=0,layersx-1
		...
		do 3 iy=0,layersy-1
			...
			do 5 k=1,3
5		           xlat(nmol,k,1)=unit_cell(ila,iat,k)+lattice_vec(k)*ixyz(k)
c		   	write(97,*)'iz,ix,iy,iat',iz,ix,iy,iat
c		   	write(97,*)'nmol',nmol
c		   	write(97,*)(xlat(nmol,k,1)*angstr_au,k=1,3)
4		    continue
3	        continue
2	    continue
1	continue
...
7	   continue
c	   if(nm.lt.6) write(99,*)'nm=',nm,(xlat(nm,k)*angstr_au,k=1,3)
6	continue
	return
	end
...
c Kelvin - au
	kelvin_au=3.163e-6
c kcal/mol - au
	kcalmol_au=1.5931e-3
c milli ev - au
	milliev_au=3.6752e-5
c amu_au - a.u. mass
	amu_au=1822.84
c angstrom - au
	angstr_au=0.52917
c au - second
	au_sec=2.4189e-17
c au - picosecond
	au_picosec=2.4189e-5
        pi=4.0d0*atan(1.0d0)
c atomic masses
	atom_mass(1)=15.995d0*amu_au
	atom_mass(2)=1.008d0*amu_au
	atom_mass(3)=1.008d0*amu_au
	atom_mass_sum=atom_mass(1)+atom_mass(2)+atom_mass(3)
...
c "dipole moment" at center of bond
	rom=0.15d0/angstr_au
	rom=0.0d0
	ao2=695000.0d0*kcalmol_au/angstr_au**12
	ao2=629400.0d0*kcalmol_au/angstr_au**12
	co2=600.0d0*kcalmol_au/angstr_au**6
	co2=625.5d0*kcalmol_au/angstr_au**6
	a12=ao2*12.0d0
	c6=co2*6.0d0
	qm=-2.0*tips2_hchg
	qmm=qm**2
...
c	read*,roo
	...
	roo=roo/angstr_au
c O-H distance
	oh_dist=0.9572d0/angstr_au
	oh_dist=1.0000d0/angstr_au
	rroh=roo*0.5d0
	rati=(oh_dist/rroh)**2
	do k=1,3
	  do kk=1,3
...
c HOH angle
	hoh_angle=104.52 *pi/180.0d0
	hoh_angle=109.47 *pi/180.0d0
	rhh=oh_dist*sin(hoh_angle/2.0d0)*2.0d0
	rrhh=rroh*sin(hoh_angle/2.0d0)*2.0d0
	print*,'rhh=',rhh*angstr_au
	rsu=oh_dist*cos(hoh_angle/2.0d0)*2.0d0
	rrsu=rroh*cos(hoh_angle/2.0d0)*2.0d0
	rsu2=rsu*0.5d0
...
c corot - coordinates of atoms in body fixed rotating system
	do 320 i=1,9
320	corot(i)=0.0d0
	aa=2.0d0*atom_mass(2)*rsu2/atom_mass_sum
	corot(3)=aa
	corot(5)=-rhh/2.0d0
	corot(6)=aa-rsu2
	corot(8)=rhh/2.0d0
...
c XINER MOMENTS OF INERTIA X Y Z = 1 2 3
	xiner_inertia(3)=atom_mass(2)*rhh**2/2.0d0
	xiner_inertia(2)=2.0d0*atom_mass(1)*atom_mass(2)*rsu2**2/atom_mass_sum
	xiner_inertia(1)=xiner_inertia(2)+xiner_inertia(3)
c QUANTITIES FOR SHAKE
	xredi(1)=2.0d0/atom_mass(1)+2.0d0/atom_mass(2)
	xredi(2)=2.0d0/atom_mass(2)+2.0d0/atom_mass(3)
	xredi(3)=2.0d0/atom_mass(1)+2.0d0/atom_mass(3)
	xdis2(1)=oh_dist**2
	xdis2(2)=rhh**2
	xdis2(3)=oh_dist**2
	do 26 ic=1,3
	...
	ic1=mod(ic,3)
	ksha(ic,2)=-1+ic1*3
	xinv(ic,1)=1.0d0/atom_mass(ic)
	xinv(ic,2)=1.0d0/atom_mass(ic1+1)
26	continue
	do 31 i=1,3
31	xdee(i)=1.0/xdis2(i)
c OO distance in crystal cubic, eisenberg+kauzmann
c	dipli=tips2_hchg*(0.5*roo-oh_dist)*tips2_hchg
c	ddpl=(0.5*roo-oh_dist)
c	dipli=tips2_hchg*(0.5*roo-oh_dist)*tips2_hchg*2*rsu2
	dipli=(tips2_hchg*2*rsu2)**2
	dne=sqrt(3.0d0)*0.25d0
	rfcc=roo/dne
c nn_cutoff is cutoff for nearest neighbor distance, nn_cutoff_sq - squared
	nn_cutoff=(roo+0.2)
	nn_cutoff_sq=nn_cutoff**2
c	print*,'cutoff for potential [A]?'
c	read*,potent_cutoff
c	write(3,99)potent_cutoff
        potent_cutoff=acutt
99	format('cutoff dist.[A]:',f15.3)
	potent_cutoff=potent_cutoff/angstr_au
	cut2=potent_cutoff**2
	return
	end
...
305	format(2i5,2a5,3f10.5,2a5,f10.5)
	...
	do 303 nm=1,nmol
	   ...
	   if(iho.eq.1)then
	      do 3306 j=1,3
3306	         ve(j)=xlat(nm,io+j,1)*angstr_au*0.5
	      write(99,305)kaa,nm,wat,car,ve,wat,car,dble(ia)
	   else
	      do 306 j=1,3
306	         ve(j)=xlat(nm,io+j,1)*angstr_au
...
10	  	continue
	   	d2=ve(1)**2+ve(2)**2+ve(3)**2
	        if(d2.lt.nn_cutoff_sq)then
c		   if(nm.eq.1)then
c			d22=sqrt(d2)*angstr_au
c		 	ctn22=sqrt(nn_cutoff_sq)*angstr_au
c			print*,d22,ctn22,nm,nmm
c			write(98,*)nm,nmm,d22,ctn22
c			write(98,*)(xlat(nm,k,1)*angstr_au,k=1,3)
c			write(98,*)(xlat(nmm,k,1)*angstr_au,k=1,3)
c		   end if
		   nne(nm)=nne(nm)+1
...
c HOH angle set to "true" water value hoh_angle, not tetrahedral 109
...
c for dipolar potl. approximation
	dimension rh(10),lattice_vec(2,3),bis(3),vhh(3),box(3,100),bmax(3)
...
15	   continue
	   ...
	   sh=0.0d0
	   do 18 j=1,3
		bis(j)=lattice_vec(1,j)+lattice_vec(2,j)
		sb=sb+bis(j)**2
		vhh(j)=lattice_vec(1,j)-lattice_vec(2,j)
		sh=sh+vhh(j)**2
18	   continue
	   sb1=1.0/sqrt(sb)
	   sh1=1.0/sqrt(sh)
...
c check potential energy
	call epot
	ep1=eppp/nmol/kcalmol_au
	eh1=ehh/nmol/kcalmol_au
	eld1=eldd/nmol/kcalmol_au
...
c	print*,'finished epot'
	eppp=ep
c	do 111 nm=1,nmol
c111	write(199,*)nm,eppo(nm)/kcalmol_au
	return
	end
